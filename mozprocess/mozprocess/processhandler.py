...
class ProcessHandlerMixin(object):
    ...
    *args* is a list of arguments to pass to the command (defaults to None).

    *cwd* is the working directory for cmd (defaults to None).

    *env* is the environment to use for the process (defaults to os.environ).

    *ignore_children* causes system to ignore child processes when True,
    defaults to False (which tracks child processes).

    *processOutputLine* is a function to be called for each line of output
    produed by the process (defaults to None).

    *onTimeout* is a function to be called when the process times out.

    *onFinish* is a functoin to be called when the process terminates normally
    without timing out.

    *kwargs* are additional keyword args to pass directly into Popen.
    ...
    class Process(subprocess.Popen):
        """
        A subclass of subprocess.Popen with better handling of child processes, 
...
falling back to not using job objects for managing child processes"""
    ...
    def commandline(self):
        """The string value of the command line (cmd + args)."""
    ...
    def kill(self):
        """
          ...
          If you created the process with
          *ignore_children=False* (the default) then it will also
          kill all child processes spawned by it.
